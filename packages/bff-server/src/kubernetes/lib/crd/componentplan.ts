...
export const COMPONENTPLAN_CRD_SPEC = {
  kind: 'ComponentPlan',
...
const { kind, group, version, plural } = COMPONENTPLAN_CRD_SPEC;
const DEFAULT_BODY = {
  kind,
...
export class ComponentPlan {
   ...
   * @param {CRD.ComponentPlan} body ComponentPlan 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  create(namespace: string, body: CRD.ComponentPlan, options?: CreateOptions) {
   ...
   * 替换指定的 ComponentPlan
   *
   * @param {string} name ComponentPlan 名称
   * @param {string} namespace 命名空间
   * @param {CRD.ComponentPlan} body ComponentPlan 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  replace(name: string, namespace: string, body: CRD.ComponentPlan, options?: CreateOptions) {
  ...
  /**
   * 部分更新指定的 ComponentPlan (JSON Patch)
   *
   * @param {string} name ComponentPlan 名称
   * @param {string} namespace 命名空间
   * @param {JsonPatchOp[]} body ComponentPlan 对象
  ...
  /**
   * 部分更新指定的 ComponentPlan (Merge Patch)
   *
   * @param {string} name ComponentPlan 名称
   * @param {string} namespace 命名空间
   * @param {object} body ComponentPlan 对象
   * @param {PatchOptions} [options] 可选配置项
   */
  ...
  /**
   * 部分更新指定的 ComponentPlan (Strategic Merge Patch)
   *
   * @param {string} name ComponentPlan 名称
   * @param {string} namespace 命名空间
   * @param {object} body ComponentPlan 对象
  ...
  deleteCollection(namespace: string, options?: DeleteCollectionOptions) {
    ...
    return this.k8sApi.deleteCollectionNamespacedCustomObject(
      ...
      response: http.IncomingMessage;
      body: CRD.ComponentPlanList;
    }>;
  }

  /**
   * 根据名称获取 ComponentPlan 详情 (指定 namespace 下)
   *
   * @param {string} name ComponentPlan 名称
   * @param {string} namespace 命名空间
