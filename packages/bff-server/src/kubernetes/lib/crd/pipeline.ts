...
export const PIPELINE_CRD_SPEC = {
  kind: 'Pipeline',
...
const { kind, group, version, plural } = PIPELINE_CRD_SPEC;
const DEFAULT_BODY = {
  kind,
...
export class Pipeline {
   ...
   * @param {CRD.Pipeline} body Pipeline 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  create(namespace: string, body: CRD.Pipeline, options?: CreateOptions) {
   ...
   * 替换指定的 Pipeline
   *
   * @param {string} name Pipeline 名称
   * @param {string} namespace 命名空间
   * @param {CRD.Pipeline} body Pipeline 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  replace(name: string, namespace: string, body: CRD.Pipeline, options?: CreateOptions) {
  ...
  /**
   * 部分更新指定的 Pipeline (JSON Patch)
   *
   * @param {string} name Pipeline 名称
   * @param {string} namespace 命名空间
   * @param {JsonPatchOp[]} body Pipeline 对象
  ...
  /**
   * 部分更新指定的 Pipeline (Merge Patch)
   *
   * @param {string} name Pipeline 名称
   * @param {string} namespace 命名空间
   * @param {object} body Pipeline 对象
   * @param {PatchOptions} [options] 可选配置项
   */
  ...
  /**
   * 部分更新指定的 Pipeline (Strategic Merge Patch)
   *
   * @param {string} name Pipeline 名称
   * @param {string} namespace 命名空间
   * @param {object} body Pipeline 对象
  ...
  deleteCollection(namespace: string, options?: DeleteCollectionOptions) {
    ...
    return this.k8sApi.deleteCollectionNamespacedCustomObject(
      ...
      response: http.IncomingMessage;
      body: CRD.PipelineList;
    }>;
  }

  /**
   * 根据名称获取 Pipeline 详情 (指定 namespace 下)
   *
   * @param {string} name Pipeline 名称
   * @param {string} namespace 命名空间
