...
export const SUBSCRIPTION_CRD_SPEC = {
  kind: 'Subscription',
...
const { kind, group, version, plural } = SUBSCRIPTION_CRD_SPEC;
const DEFAULT_BODY = {
  kind,
...
export class Subscription {
   ...
   * @param {CRD.Subscription} body Subscription 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  create(namespace: string, body: CRD.Subscription, options?: CreateOptions) {
   ...
   * 替换指定的 Subscription
   *
   * @param {string} name Subscription 名称
   * @param {string} namespace 命名空间
   * @param {CRD.Subscription} body Subscription 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  replace(name: string, namespace: string, body: CRD.Subscription, options?: CreateOptions) {
  ...
  /**
   * 部分更新指定的 Subscription (JSON Patch)
   *
   * @param {string} name Subscription 名称
   * @param {string} namespace 命名空间
   * @param {JsonPatchOp[]} body Subscription 对象
  ...
  /**
   * 部分更新指定的 Subscription (Merge Patch)
   *
   * @param {string} name Subscription 名称
   * @param {string} namespace 命名空间
   * @param {object} body Subscription 对象
   * @param {PatchOptions} [options] 可选配置项
   */
  ...
  /**
   * 部分更新指定的 Subscription (Strategic Merge Patch)
   *
   * @param {string} name Subscription 名称
   * @param {string} namespace 命名空间
   * @param {object} body Subscription 对象
  ...
  deleteCollection(namespace: string, options?: DeleteCollectionOptions) {
    ...
    return this.k8sApi.deleteCollectionNamespacedCustomObject(
      ...
      response: http.IncomingMessage;
      body: CRD.SubscriptionList;
    }>;
  }

  /**
   * 根据名称获取 Subscription 详情 (指定 namespace 下)
   *
   * @param {string} name Subscription 名称
   * @param {string} namespace 命名空间
